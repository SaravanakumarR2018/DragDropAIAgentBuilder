...
def run_command(command: list[str], cwd: pathlib.Path) -> str:
    # `command` comes from a trusted source (Makefile), safe to ignore S603
...
def parse_revision(lines: Iterable[str]) -> str:
    ...
    msg = "Could not parse Alembic revision from command output"
...
def main() -> None:
    ...
    if args.versions_dir:
        ...
        if not versions_dir.exists():
            msg = f"Alembic versions directory not found: {versions_dir}"
            raise SystemExit(msg)
