...
| <Link id="LANGFLOW_FEATURE_MULTI_ORG_DB_OLD"/><span class="env-prefix">LANGFLOW_</span>FEATURE_MULTI_ORG_DB | Boolean | `true` | If set to `true`, this enables multi-organization database support using Clerk for authentication and `org_id` extraction from JWTs. Langflow will use an `OrgDatabaseMiddleware` to extract an `org_id` from the incoming JWT. This `org_id` is then available on `request.state.org_id` for use in scoping database queries or resources via `LANGFLOW_DATABASE_URL_TEMPLATE`. <br/>**Related Configuration:** Requires `CLERK_FRONTEND_API_URL` to be set for fetching JWKS. `CLERK_JWT_AUDIENCE` and `CLERK_ALGORITHM` (defaults to "RS256") might also be needed depending on your Clerk setup. This variable is distinct from the newer `LANGFLOW_FEATURE_MULTI_ORG_DB` used for the session-based multi-tenancy. |
