...
export default function OrganizationOnboarding() {
  ...
  }, [getToken, isSignedIn, organization?.id, searchParams, setSearchParams]);
  ...
  useEffect(() => {
    ...
    if (organization?.id && !bootstrappedRef.current) {
      const selectedFlag = processedOrgRef.current === organization.id;
      if (selectedFlag) {
        bootstrapSession();
      }
    }
    ...
    <div className="login-shell">
      <div className="login-grid">
        <section className="login-card login-card--primary">
          ...
          <h1 className="login-heading">Choose your organization</h1>
          <p className="login-subheading">
            Pick the Clerk organization you'd like to open in Langflow. We'll provision everything on our backend automatically.
          </p>
          {error && (
        ...
        <section className="login-card">
          <div className="login-card-header">
            <div>
              <p className="login-eyebrow">Organization roster</p>
              <h2 className="login-heading-small">Select or create a workspace</h2>
            </div>
          </div>
          <p className="login-subheading">
            After you select or create an organization, we'll call Langflow's <code>create_organisation</code> endpoint to ensure your backend workspace exists.
          </p>
          <SignedIn>
          ...
          <SignedOut>
            <div className="login-alert login-alert--info">
              Your session expired. Return to the login page to sign in again.
            </div>
          </SignedOut>
        </section>
      </div>
