...
import {
  API_ROUTES,
  // BASENAME,  // ⟵ not used for frontend1 to avoid accidental non-root base
  PORT,
  PROXY_TARGET,
...
export default defineConfig(({ mode }) => {
  ...
  const proxyTargets = apiRoutes.reduce((proxyObj: Record<string, any>, route: string) => {
    proxyObj[route] = {
      target: target,
      changeOrigin: true,
      secure: false,
  ...
  return {
    // Frontend1 owns "/", "/login", "/organization" → must be rooted at "/"
    base: "/",
    ...
    build: {
      ...
      rollupOptions: {
        output: {
          // hashed filenames for safe long-term caching behind nginx
