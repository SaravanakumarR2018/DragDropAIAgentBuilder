...
async def init_mcp_servers():
    ...
    try:
        # Initialize global MCP servers first (non-organisation DB)
        async with session_scope(use_organisation=False) as session:
            settings_service = get_settings_service()

        # Then initialize organisation-specific MCP servers
        async with session_scope() as session:
            ...
            for project in projects:
                try:
                    ...
                    # Handle disabled Composer + OAuth cleanup
                    if (
                    ...
                    # Register with MCP Composer if OAuth is configured
        ...
        # ✅ Initialize org-specific MCP servers after global setup
        try:
