...
def upgrade() -> None:
    ...
    conn = op.get_bind()
    inspector = sa.inspect(conn) if conn is not None else None  # type: ignore
    # List existing tables
    existing_tables = inspector.get_table_names()
    # Drop 'flowstyle' table if it exists
    # and other related indices
    if inspector and "flowstyle" in existing_tables:
    ...
    existing_fks_flow = []
    if inspector and "flow" in existing_tables:
    ...
    with op.batch_alter_table("flow", schema=None) as batch_op:
        flow_columns = [col["name"] for col in inspector.get_columns("flow")] if inspector else []
